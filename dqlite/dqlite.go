...
func (dialector Dialector) Initialize(db *gorm.DB) (err error) {
	...
	if dialector.Conn != nil {
		...
		logFunc := func(l client.LogLevel, format string, a ...interface{}) {
			...
			fn().Str(DriverName, fmt.Sprintf(format, a...))
		}

		c := strings.Split(os.Getenv("DQLITE_CLUSTER_NODES"), ",")

		app, err := app.New(
			os.Getenv("DQLITE_DB_PATH"),
			app.WithAddress(os.Getenv("DQLITE_NODE_ADDRESS")),
			app.WithCluster(c),
